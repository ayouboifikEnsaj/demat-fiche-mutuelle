...
import { Fiche, FicheUpdateDto, ActionUpdateDto, DetailsActionUpdateDto } from '../../models/Fiche';
...
export class FicheEditComponent implements OnInit {
  isComplementMode: boolean = false;
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(params => {
      this.isComplementMode = params['mode'] === 'complement';
    });
    this.loadFiche();
  }
  ...
  envoyerComplement(): void {
    ...
    this.loading = true;
    this.ficheService.ajouterComplement(this.ficheId, formData).subscribe({
      next: () => {
        this.loading = false;
...
// ⚠️ MODIFIER loadFiche() pour charger toutes les sections fixes :
  loadFiche(): void {
